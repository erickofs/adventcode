...
# define cline as input.readline
# Define sf as 0
# While cline exists:
    # Split cline by 'x' and assign to l, w, h
    # Perform 2*l*w + 2*w*h + 2*h*l + the smallest side and add to sf
...
with open('input.txt') as input:
    ...
    while cline:
        ...
        cline = input.readline()
    print(f'The total amount of square feet of wrapping paper needed is {sf}.')
...
with open('input.txt') as input:
    ...
    while cline:
        l, w, h = cline.split('x')
        l, w, h = int(l), int(w), int(h)
        rb += 2*min(l+w, w+h, h+l) + l*w*h
        cline = input.readline()
    print(f'The total amount of feet of ribbon needed is {rb}.')